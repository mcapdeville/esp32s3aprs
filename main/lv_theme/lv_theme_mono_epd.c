 ...
 *
 * main/lv_theme/lv_theme_mono_epd.c
 *
...
lv_theme_t * lv_theme_mono_epd_init(lv_disp_t * disp, bool dark_bg, const lv_font_t * font)
{
    ...
    if(!inited) {
	styles = lv_mem_alloc(sizeof(my_theme_styles_t));
	if (!styles)
		return NULL;
    }
